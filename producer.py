...
def fetch_data():
    ...
    # FIX TIMEZONE (New York → IST)
    df["time"] = pd.to_datetime(df["datetime"])
    df["time"] = df["time"].dt.tz_localize(NY).dt.tz_convert(IST)
...
while True:
    ...
    try:
        ...
        for step in range(PRED_MINUTES):
            ...
            pred_ret = pred_scaled * volatility
            pred_ret = np.clip(pred_ret, -2*volatility, 2*volatility)

            pred_price = last_price * (1 + pred_ret)

            # volatility noise
            pred_price *= (1 + np.random.normal(0, volatility/3))
